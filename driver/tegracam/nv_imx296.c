...
/* Enables dev_dbg(), a full register dump on every mode-set/start/stop, and a
 * msleep(1000) in start_streaming - together ~1.3s of added latency per stream
 * start. Useful during bring-up, off for normal use.
 *
 * Must precede all includes: dev_dbg()'s macro expansion in dev_printk.h keys
 * off DEBUG at the point <linux/module.h> is included below. */
/* #define DEBUG */
...
/*
 * The VMAX that imx296_common_regs actually programs (0x3010-0x3012 = 1118).
 * imx296_set_mode records this as the active frame length so the exposure
 * maths matches the sensor. Must track the mode table - if the table's VMAX
 * changes, change this with it.
 */
#define IMX296_VMAX_TABLE 1118
...
/*
 * Program VMAX and record it as the frame length the rest of the driver
 * works against. imx296_set_exposure derives SHS1 = frame_length -
 * coarse_time, so the sensor and priv->frame_length must never disagree.
 */
static int imx296_write_frame_length(struct imx296 *priv, u32 frame_length)
{
	...
	for (i = 0; i < IMX296_VMAX_REG_COUNT; i++) {
		err = imx296_write_reg(priv->s_data, reg_list[i].addr,
				       reg_list[i].val);
		if (err)
			return err;
	}
...
static int imx296_set_frame_rate(struct tegracam_device *tc_dev, s64 val)
{
	...
	err = imx296_write_frame_length(priv, frame_length);
	if (err) {
	...
	/* VMAX changed; SHS1 is relative to VMAX, so re-apply last exposure. */
	if (priv->last_exposure_valid)
		return imx296_set_exposure(tc_dev, priv->last_exposure_us);
...
static int imx296_set_exposure(struct tegracam_device *tc_dev, s64 val)
{
     ...
     *
     * val arrives in plain microseconds: the device tree's min_exp_time /
     * max_exp_time are microseconds, and exposure_factor (1000000) is only
     * the scale that turns pix_clk_hz into those microseconds - it is not an
     * extra multiplier on val.
     *
     * The readout offset must therefore be subtracted in microseconds too.
     * Scaling it by exposure_factor made (val - 14000000) negative for every
     * value in the 15..15699us control range, and because pixel_clock.val is
     * __u64 the expression was then evaluated in unsigned 64-bit and wrapped,
     * producing a huge coarse_time that always clamped to the maximum. The
     * net effect was SHS1 == 4 - exposure pinned wide open - no matter what
     * was requested.
     */
	exp_us = val - IMX296_EXPOSURE_OFFSET_US;
	if (exp_us < 0)
		exp_us = 0;

	coarse_time = (u32)(mode->signal_properties.pixel_clock.val * exp_us /
...
static int imx296_set_mode(struct tegracam_device *tc_dev)
{
	...
	dev_info(dev,
		 "DIAG: mode=%d mode_prop_idx=%d sensor_mode_id=%d num_modes=%d numfmts=%d def_mode=%d fmt=%dx%d\n",
		 s_data->mode, s_data->mode_prop_idx, s_data->sensor_mode_id,
		 s_data->sensor_props.num_modes, s_data->numfmts,
		 s_data->def_mode, s_data->fmt_width, s_data->fmt_height);
	...
	/*
     * imx296_common_regs unconditionally programs VMAX = IMX296_VMAX_TABLE,
     * overriding anything imx296_set_frame_rate wrote earlier. The sensor is
     * therefore running that value now, so record it: imx296_set_exposure
     * derives SHS1 = frame_length - coarse_time, and if priv->frame_length
     * still held set_frame_rate's number (1125 at 60fps) every integration
     * came out short by the difference - 7 lines, negligible at long
     * exposures but a large relative error at short ones.
     *
     * Do not instead push priv->frame_length into VMAX here. The capture
     * pipeline is configured from the device tree for the mode table's
     * timing, and retiming the sensor behind its back stalls the VI channel:
     * raw captures then block forever waiting for buffers that never arrive.
     */
	priv->frame_length = IMX296_VMAX_TABLE;

	/* set_mode rewrote VMAX; SHS1 is stale until exposure is reapplied. */
	if (priv->last_exposure_valid) {
		err = imx296_set_exposure(tc_dev, priv->last_exposure_us);
		if (err)
			return err;
	}
...
static int imx296_start_streaming(struct tegracam_device *tc_dev)
{
	 ...
	 * Hardware power-on leaves PGCTRL=0x06 (CLKEN|THRU). THRU bypasses
	 * the pixel array, so it must be cleared for live capture.
	 * Mainline imx296.c disables TPG by writing CLKEN only (0x04).
	 * Writing 0 also drops CLKEN and the analog readout collapses into
	 * uncorrelated 0 / 0xFFFF snow.
	 */
	if (test_mode) {
		...
		err = imx296_write_reg(s_data, IMX296_PGCTRL_ADDR,
				       IMX296_PGCTRL_CLKEN);
		if (err)
			goto fail;
	}
